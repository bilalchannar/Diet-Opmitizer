...
class DietOptimizerGUI:
    ...
    def create_widgets(self):
        ...
        gender_combo = ttk.Combobox(user_frame, textvariable=self.gender_var, values=["male", "female"], width=8, state="readonly")
        ...
        activity_combo = ttk.Combobox(user_frame, textvariable=self.activity_var, values=["sedentary", "light", "moderate", "active", "very_active"], width=10, state="readonly")
        ...
        goal_combo = ttk.Combobox(user_frame, textvariable=self.goal_var, values=["lose", "maintain", "gain"], width=10, state="readonly")
        ...
        ttk.Button(user_frame, text="Calculate BMI", command=self.calculate_bmi).grid(row=7, column=0, columnspan=2, pady=5)
        ...
        for nutrient, values in DEFAULT_TARGETS.items():
            ...
            row += 1
        ttk.Button(targets_frame, text="Auto-Calculate Targets", command=self.auto_calculate_targets).grid(row=row, column=0, columnspan=4, pady=5)
        btn_frame = ttk.Frame(left_frame)
        btn_frame.grid(row=3, column=0, sticky="ew", pady=10)
        self.start_btn = ttk.Button(btn_frame, text="🚀 Start Optimization", command=self.start_optimization)
        self.start_btn.pack(fill="x", pady=2)
        self.stop_btn = ttk.Button(btn_frame, text="⏹️ Stop", command=self.stop_optimization, state="disabled")
        self.stop_btn.pack(fill="x", pady=2)
        ttk.Button(btn_frame, text="📊 Show Visualization", command=self.show_visualization).pack(fill="x", pady=2)
        ttk.Button(btn_frame, text="💾 Export to CSV", command=self.export_csv).pack(fill="x", pady=2)
        self.progress_var = tk.DoubleVar()
        self.progress = ttk.Progressbar(left_frame, variable=self.progress_var, maximum=100, mode='determinate')
        ...
        self.results_text = scrolledtext.ScrolledText(results_tab, wrap=tk.WORD, font=("Consolas", 10))
        ...
        self.progress_text = scrolledtext.ScrolledText(progress_tab, wrap=tk.WORD, font=("Consolas", 9))
    ...
    def populate_foods_tree(self):
        for name, info in FOOD_DATABASE.items():
            self.foods_tree.insert("", "end", values=(name, info["calories"], info["protein"], info["carbs"], info["fat"], info["fiber"], f"${info['price']:.2f}", info["category"]))
    ...
    def get_targets(self):
        ...
        for nutrient, (min_var, max_var) in self.target_vars.items():
            try:
                targets[nutrient] = {"min": float(min_var.get()), "max": float(max_var.get()), "weight": DEFAULT_TARGETS[nutrient]["weight"]}
            except ValueError:
    ...
    def update_progress(self, generation, stats, best_individual):
        ...
        self.progress_text.insert("end", f"Gen {generation:4d}: Best={stats['best']:.2f}, Avg={stats['avg']:.2f}\n")
    ...
    def run_optimization(self):
        try:
            ...
            self.ga = GeneticAlgorithm(population_size=int(self.pop_var.get()), num_generations=int(self.gen_var.get()), crossover_rate=float(self.cross_var.get()), mutation_rate=float(self.mut_var.get()), targets=targets, budget=budget)
            self.ga.set_generation_callback(lambda g, s, b: self.root.after(0, self.update_progress, g, s, b))
    ...
    def export_csv(self):
        if self.best_diet:
            ...
            filename = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV files", "*.csv")], initialfilename="diet_plan.csv")
            if filename:
